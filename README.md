...
当前 `bundle.active` 为 `false`，因此默认只生成可执行文件，不生成 MSI 或 NSIS 安装包。前端资源会嵌入程序，无需单独分发 `dist`。发布体积会随编译器和依赖变化；当前本地 release 构建约为 8.3 MiB。
...
需要截取网页、文档或聊天记录时，先框选其中稳定的可滚动内容区域，再在编辑器点击 `长截图`。SnapRust 会隐藏编辑器、激活选区下方的窗口并自动向下滚动，通过相邻画面的重叠部分完成拼接；到达底部后会回到编辑器，长图仍可继续标注、裁剪、OCR、翻译、复制或钉图。执行期间请不要滚动窗口或遮挡选区。
...
```text
snaprust-translation.json       翻译提供商、端点、模型和 API Key
history/                       截图历史
...
- 翻译配置目前保存在本地 JSON 文件中，API Key 并未使用 Windows Credential Manager 加密。请保护好 Windows 账户和应用数据目录。
...
- 长截图适用于内容位置稳定的纵向滚动区域，最多拼接 24 段、40,000 像素高且不超过 6,000 万像素。视频、动画、吸顶/悬浮元素、横向滚动、需要管理员权限的窗口或不响应 Windows 滚轮消息的软件可能无法可靠拼接。
