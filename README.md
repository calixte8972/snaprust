...
需要截取网页、文档或聊天记录时，先框选其中稳定的可滚动内容区域，再在编辑器点击 `长截图`。SnapRust 会隐藏编辑器、激活选区下方的窗口并自动向下滚动，通过相邻画面的重叠部分完成拼接；到达底部后会回到编辑器，长图仍可继续标注、裁剪、OCR、翻译、复制或钉图。执行期间请不要滚动窗口或遮挡选区，按 `Esc` 可以取消。
